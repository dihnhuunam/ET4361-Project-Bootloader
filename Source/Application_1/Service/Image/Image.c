/**
 * @file Image.c
 * @brief Implements application image validation and CRC checking.
 */
#include "Image.h"
...
/**
 * @brief Resets an image validation result object to its default state.
 ...
 * @param
 * stack_pointer_out Optional output for the initial stack pointer.
 * @param reset_handler_out Optional output for the
 * reset handler address.
 * @return true if the vector table is valid, otherwise false.
 */
bool Image_IsVectorTableValid(BootSlot_t slot, uint32_t base_address,
                              uint32_t image_size, uint32_t *stack_pointer_out,
                              uint32_t *reset_handler_out) {
  uint32_t stack_pointer;
  uint32_t reset_handler;
  uint32_t reset_handler_address;

  if (!BootConfig_IsAppRangeValid(slot, base_address, image_size) ||
      (image_size < 8U)) {
    return false;
  }
 ...
 */
HAL_StatusTypeDef Image_ComputeCrc(BootSlot_t slot, uint32_t image_size,
                                   uint32_t *crc_out) {
  const BootSlotRegion_t *region;

  region = BootConfig_GetSlotRegion(slot);
  if ((region == 0) || (crc_out == 0) || (image_size == 0U) ||
...
bool Image_Validate(BootSlot_t slot, uint32_t image_size, uint32_t expected_crc,
  ...
  if (!Image_IsVectorTableValid(
          slot, region->start_address, image_size,
