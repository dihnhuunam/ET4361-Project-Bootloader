...
static HAL_StatusTypeDef Bootloader_ReceiveExact(uint8_t *buffer,
                                                 uint32_t length) {
...
static BootSlot_t
Bootloader_SelectRecoverySlot(const BootMetadata_t *metadata) {
  ...
  if ((metadata->active_slot == BOOT_SLOT_APP1) ||
      (metadata->active_slot == BOOT_SLOT_APP2)) {
    /* Keep the currently active firmware untouched and recover into the other
     * slot. */
...
static void Bootloader_RunRecoveryMode(BootMetadata_t *metadata) {
  ...
  Debug("Target slot=%lu addr=0x%08lX size=%lu\r\n", (unsigned long)slot,
        (unsigned long)region->start_address, (unsigned long)region->size);
  ...
  while (1) {
    ...
    if (memcmp(header.magic, BOOTLOADER_RECOVERY_MAGIC, sizeof(header.magic)) !=
        0) {
    ...
    Debug("Recovery erase slot=%lu image_size=%lu crc=0x%08lX version=%lu\r\n",
          (unsigned long)slot, (unsigned long)header.image_size,
          (unsigned long)expected_crc, (unsigned long)header.version);

    /* Erase the full target slot so the new image is written into a clean
     * region. */
    ...
    while (remaining > 0U) {
      uint32_t chunk_size = (remaining > BOOTLOADER_RECOVERY_CHUNK_SIZE)
                                ? BOOTLOADER_RECOVERY_CHUNK_SIZE
                                : remaining;
    ...
    if (calculated_crc != expected_crc) {
      Debug("Recovery crc mismatch calc=0x%08lX expected=0x%08lX\r\n",
            (unsigned long)calculated_crc, (unsigned long)expected_crc);
      continue;
    }

    /* Mark the new slot as pending so the normal boot flow can trial-boot it.
     */
    status = Metadata_MarkPending(slot, header.image_size, expected_crc,
                                  header.version);
    if (status != HAL_OK) {
...
static void Bootloader_EnterSafeMode(const char *reason) {
  ...
  /* In safe mode, stay in UART recovery until a valid firmware image is
   * received. */
...
/**
 * @brief Selects a bootable slot only when the slot state matches the expected
 * values.
 ...
 */
static bool Bootloader_IsSlotBootableWithStates(
    const BootMetadata_t *metadata, BootSlot_t slot, uint32_t state_a,
  ...
  return Image_Validate(slot, slot_metadata->image_size,
                        slot_metadata->image_crc, result_out);
}
 ...
 */
static BootSlot_t
Bootloader_SelectConfirmedSlot(const BootMetadata_t *metadata) {
  ...
  if (((metadata->previous_slot == BOOT_SLOT_APP1) ||
       (metadata->previous_slot == BOOT_SLOT_APP2)) &&
  ...
  for (index = 0U; index < count; index++) {
    if (Bootloader_IsSlotBootableWithStates(metadata, candidates[index],
                                            SLOT_STATE_CONFIRMED,
                                            SLOT_STATE_VALID, &validation)) {
 ...
 */
static bool Bootloader_HandlePending(BootMetadata_t *metadata,
                                     BootSlot_t *slot_out) {
  ...
  if (!Bootloader_IsSlotBootableWithStates(metadata, pending_slot,
                                           SLOT_STATE_PENDING,
                                           SLOT_STATE_PENDING, &validation)) {
