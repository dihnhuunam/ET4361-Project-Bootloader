...
#define BOOT_FLASH_END_ADDRESS                                                 \
  (BOOT_FLASH_BASE_ADDRESS + BOOT_FLASH_TOTAL_SIZE - 1UL)

#define BOOT_SRAM_BASE_ADDRESS 0x20000000UL
#define BOOT_SRAM_TOTAL_SIZE (20UL * 1024UL)
#define BOOT_SRAM_END_ADDRESS                                                  \
  (BOOT_SRAM_BASE_ADDRESS + BOOT_SRAM_TOTAL_SIZE - 1UL)

#define BOOTLOADER_REGION_START 0x08000000UL
#define BOOTLOADER_REGION_SIZE (32UL * 1024UL)
#define BOOTLOADER_REGION_END                                                  \
  (BOOTLOADER_REGION_START + BOOTLOADER_REGION_SIZE - 1UL)
 ...
 */
bool BootConfig_IsAppRangeValid(BootSlot_t slot, uint32_t address,
                                uint32_t length);

/**
 * @brief Checks whether an address is inside the configured SRAM window.
 * @param address Address to validate.
 * @return true if the address is inside the configured SRAM range, otherwise
 * false.
 */
