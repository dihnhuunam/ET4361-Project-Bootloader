...
static HAL_StatusTypeDef Metadata_UpdateCrc(BootMetadata_t *metadata) {
  ...
  return Crc32_CalculateBuffer((const uint8_t *)metadata,
                               sizeof(BootMetadata_t) - sizeof(uint32_t),
...
static bool Metadata_IsErasedRecord(const BootMetadata_t *metadata) {
  ...
  for (index = 0U; index < (sizeof(BootMetadata_t) / sizeof(uint32_t));
       index++) {
...
static uint32_t Metadata_GetNextWriteAddress(void) {
  ...
  for (index = 0U; index < METADATA_RECORD_COUNT; index++) {
    (void)Flash_Read(Metadata_GetRecordAddress(index), (uint8_t *)&record,
                     sizeof(record));
...
HAL_StatusTypeDef Metadata_LoadLatest(BootMetadata_t *metadata,
  ...
  for (index = 0U; index < METADATA_RECORD_COUNT; index++) {
    (void)Flash_Read(Metadata_GetRecordAddress(index), (uint8_t *)&candidate,
                     sizeof(candidate));
...
HAL_StatusTypeDef Metadata_Save(BootMetadata_t *metadata) {
  ...
  return Flash_Write(write_address, (const uint8_t *)metadata,
                     sizeof(*metadata));
}
...
HAL_StatusTypeDef Metadata_ConfirmSlot(BootSlot_t slot) {
  ...
  if (other_slot != BOOT_SLOT_NONE) {
    SlotMetadata_t *other_metadata =
        Metadata_GetSlotMutable(&metadata, other_slot);
    if ((other_metadata != 0) &&
        (other_metadata->state == SLOT_STATE_PENDING)) {
